...
const AdminLayout = () => {
  ...
  const [openSubmenus, setOpenSubmenus] = useState(() => {
    const isBooksActive = location.pathname.includes("/admin/books") || 
                          ...
                          location.pathname.includes("/admin/authors") || 
                          location.pathname.includes("/admin/upload") || 
                          location.pathname.includes("/admin/edit-book");
    return { books: isBooksActive };
  });
  ...
  const menuItems = [
    ...
    { 
      ...
      label: "Quản lý sách",
      subItems: [
        { path: "/admin/books", icon: Book, label: "Danh sách sách" },
        { path: "/admin/categories", icon: Tag, label: "Quản lý danh mục" },
        { path: "/admin/authors", icon: Users, label: "Quản lý tác giả" },
      ]
    },
    { path: "/admin/users", icon: Users, label: "Quản lý người dùng" },
    { path: "/admin/payments", icon: CreditCard, label: "Quản lý giao dịch" },
          ...
          {menuItems.map((item) => {
            if (item.subItems) {
              const isActive = item.subItems.some(sub => location.pathname === sub.path || location.pathname.startsWith(`${sub.path}/`)) || 
                               (item.id === "books" && (location.pathname.startsWith("/admin/upload") || location.pathname.startsWith("/admin/edit-book")));
                    ...
                    <div className="pl-4 space-y-1 mt-1 animate-in slide-in-from-top-2 duration-200">
                      {item.subItems.map((subItem) => {
                        const isSubActive = location.pathname === subItem.path || 
                                            (subItem.path === "/admin/books" && (
                                              location.pathname.startsWith("/admin/upload") || 
                                              location.pathname.startsWith("/admin/edit-book") || 
                                              (location.pathname.startsWith("/admin/books/") && location.pathname !== "/admin/books")
                                            ));
                        return (
                          <Link
                            key={subItem.path}
                          ...
                          >
                            <subItem.icon size={18} className={isSubActive ? "text-blue-600" : "text-slate-400"} />
