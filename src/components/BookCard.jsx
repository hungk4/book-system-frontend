...
const BookCard = ({ book }) => {
  const avgRating = book.avg_rating !== undefined ? Number(book.avg_rating) : (book.avgRating !== undefined ? Number(book.avgRating) : (book.rating !== undefined ? Number(book.rating) : 0));
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-3/4 w-full overflow-hidden bg-gray-100 dark:bg-slate-700">
        ...
        {/* Badges bên trái xếp dọc */}
        <div className="absolute top-3 left-3 flex flex-col gap-1.5 items-start z-10">
          {book.category_name && (
            <span className="bg-white/90 dark:bg-slate-900/90 backdrop-blur-sm text-blue-600 dark:text-blue-400 text-[10px] font-bold px-2 py-0.5 rounded shadow-sm">
              {book.category_name}
            </span>
          )}
        </div>
        ...
        {book.is_premium && (
          <span className="absolute top-3 right-3 bg-yellow-500 text-white text-xs font-bold px-2 py-1 rounded shadow-sm flex items-center z-10">
            <Star size={10} className="mr-1 fill-white" /> Hội viên
          </span>
        )}
      ...
      <div className="p-4 flex flex-col grow">
        ...
        <div className="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-3">
          ...
          <span className="truncate">{book.author_name || "Tác giả ẩn danh"}</span>
        </div>
