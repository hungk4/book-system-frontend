...
import { Edit, Trash2, Plus, X, Tag, List, ChevronLeft, ChevronRight, Search, ChevronDown } from "lucide-react";
...
const AdminCategoriesPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name.trim()) return toast.warn("Tên thể loại không được để trống");

    try {
      if (editingId) {
        await axiosClient.put(`/categories/${editingId}`, { name });
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Bạn có chắc chắn muốn xóa thể loại này?")) {
      try {
        ...
        toast.error("Lỗi khi xóa: Thể loại có thể đang chứa sách");
      }
    }
  };
    ...
    <div className="p-4 md:p-8 animate-fadeIn">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Thêm, sửa hoặc xóa các danh mục phân loại sách trong hệ thống.</p>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Ví dụ: Công nghệ thông tin, Văn học, Tâm lý..."
              value={name}
          ...
          <div className="flex gap-2">
            ...
            >
              ...
              {editingId ? "Cập nhật" : "Tạo thể loại"}
            </button>
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-100">
                <th className="px-6 py-4 font-semibold text-xs text-gray-500 uppercase tracking-wider">Tên thể loại</th>
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                <tr>
                  <td colSpan="3" className="px-6 py-10 text-center text-gray-400">Đang tải dữ liệu...</td>
                </tr>
              ) : categories.length > 0 ? (
                currentCategories.map((cat) => (
                  <tr key={cat.id} className="hover:bg-indigo-50/30 transition-colors group">
                    <td className="px-6 py-4">
                      <span className="font-medium text-gray-700 group-hover:text-indigo-600 transition-colors">{cat.name}</span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500 font-medium">
                      {cat.book_count || 0} cuốn sách
                    </td>
                    <td className="px-6 py-4">
                ...
                <tr>
                  <td colSpan="3" className="px-6 py-10 text-center text-gray-400 italic">
                    {searchTerm ? `Không tìm thấy thể loại nào phù hợp với từ khóa "${searchTerm}"` : "Chưa có dữ liệu thể loại nào được tạo."}
                  </td>
                </tr>
              )}
            </tbody>
        ...
        {pagination.total > 0 && (
          <div className="flex items-center justify-between px-6 py-4 border-t border-gray-100 bg-gray-50/50">
            <div className="text-sm text-gray-500">
              Hiển thị <span className="font-medium text-gray-700">{(safePage - 1) * ITEMS_PER_PAGE + 1}</span> đến <span className="font-medium text-gray-700">{Math.min(safePage * ITEMS_PER_PAGE, pagination.total)}</span> trong số <span className="font-medium text-gray-700">{pagination.total}</span> danh mục
            </div>
